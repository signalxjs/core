...
const ANSWER =
    ...
    'The component calls ssr.stream() with an async iterable — on the server each ' +
...
export const Ai = component((ctx) => {
    ...
    const answer = ctx.ssr.stream('answer', () => fakeLlmTokens());

    return () => (
        <>
            <h1>AI token streaming</h1>
            <p>Reload this page and watch the answer arrive word by word — that is the <em>server</em> streaming tokens into the initial HTML response via <code>ssr.stream()</code>, not client-side JavaScript.</p>
            <div class="card">
