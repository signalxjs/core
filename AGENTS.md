...
pnpm bench:ssr         # full comparative SSR bench: equivalence check, then sigx vs Vue/React/Preact
                       # CI runs `verify` + the quick suite (bench-smoke job) as a CORRECTNESS gate — timing
                       # enforcement is local-only, since check-regression skips it when the baseline's machine
                       # fingerprint differs. Re-baseline on YOUR machine with `pnpm bench:ssr:baseline`.
