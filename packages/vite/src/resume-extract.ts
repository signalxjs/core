 ...
 *   `($scope, <original params>) => body` — the runtime invokes it as
 *   `($scope, event)`, the arity live dispatch gives the original, with
 *   `event.currentTarget` re-pointed at the delegated element — with
 *   captured named signals rewritten to `$scope.signals.<name>` and
 *   `ctx.props` reads to `$scope.props`. The module imports nothing from this file (that would
