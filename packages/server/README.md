...
**The build refuses what it cannot represent.** In a `*.server.*` module
these are build errors, named with file and line (rfc-server-v5 §1.7): a value
re-export (`export * from`, `export { a } from`) — the client stub would
silently lack the names; a default-exported server function — its route needs
a stable export name; a server function that is not an exported module-scope
...
- **Version skew is a 409, never a wrong call**: every stub call carries its
  ...
  after a deploy). Clients that send no tag — native clients, curl, form
  posts — and hand-built `resolve` tables are simply served.
