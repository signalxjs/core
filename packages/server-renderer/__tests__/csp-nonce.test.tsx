...
describe('CSP nonce — modulepreload links', () => {
    const template = '<html><head></head><body><div id="app"><!--ssr-outlet--></div></body></html>';
    // The three sources renderAssetLinks stamps: the app's `assets`, a
    // boundary chunk recorded during the walk, and a pack's `assets()` hook.
    ...
    const pack: SSRPlugin = {
        ...
        server: {
            resolveBoundary(vnode) {
                return (vnode.type as any).__loadStamp ? { hydrate: 'load', chunk: { url: '/assets/load.js' } } : undefined;
            },
            assets: () => ({ modulepreload: ['/assets/pack.js'] })
        }
    };
    const page = () => <div><Plain /><Load /></div>;

    it('stamps the nonce on every modulepreload link — script-src governs module preloads', async () => {
        const html = await createSSR({ plugins: [pack] }).renderDocument(page(), {
            template,
            nonce: 'abc123',
        ...
        expect(html).toContain('<link rel="modulepreload" href="/assets/load.js" nonce="abc123">');
    ...
    it('emits the historical bare links without a nonce', async () => {
        ...
        expect(html).toContain('<link rel="modulepreload" href="/assets/load.js">');
