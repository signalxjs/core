...
- **Release tooling hardened for 1.0 (#363).** `scripts/verify-pack.js` now
  ...
  months; it did not exist). Every server-side entry is now imported under
  Node from the installed tarballs, in both dev and production conditions.
  ...
  (prereleases like `1.0.0-rc.0` included); any other argument — `--help`
...
- **The pipeline accessors were unreachable on workerd (#630/#631).**
  `serverFeature`, `principal`, `requirePrincipal`, `setPrincipal` and
  `requireAuthenticated` shipped in 0.15.1 on the package root only. The
  root carries a `browser` condition, and `wrangler` bundles Workers with
  that condition — so a Cloudflare deployment resolved the stubs and threw
  *"serverFeature() reached the browser unextracted"* at runtime, **on a
  genuine server**. They are now re-exported from `@sigx/server/server`,
  which has no `browser` branch, for packs whose runtime is server-only
  everywhere (`@sigx/actors` on Durable Objects is the case that surfaced
  it). The root exports are unchanged: an app's own `*.server.ts` files are
  extracted by the build and never reach a client bundle, so the root stays
  the right door for user code.
...
- **`@sigx/runtime-core` / `@sigx/server-renderer`**: a duplicated module graph no longer fails in silence (#430). `di/token.ts` has always stated that DI identity must live in one module graph and that a duplicated one "should fail loudly (provides not found)" — but it didn't: a miss is exactly what "nothing was provided" looks like, and every seam reads that as a legitimate default (`getSSRPlugins()` → `[]`, hydrate defaults → core's, type handlers → the built-ins). That ambiguity is what let #425 render resumable and island pages with no boundary table at all and say nothing, from #418 until a user reported it — and no test could have caught it, since a unit test never has two module graphs. New dev-only `hasForeignToken(provides, token)` (`sigx/internals`): the signal is a provides key carrying the token's description that is **not** the token, which reads as a duplicated graph because of the seam contract — descriptions are namespaced `sigx:*` and each is minted in exactly one place — rather than as a guarantee of JavaScript. `mergeSSRPlugins` now calls it under `__DEV__` and warns with the cause, the consequence and the usual fixes. Not wired into `getProvided` — that is the hot injection path, where a miss is ordinarily legitimate. `docs/seams.md` documents the assumption and the check.

- **`@sigx/vite`**: app-carried SSR plugins are no longer invisible to the dev renderer (#425). `createDevRequestHandler` loaded the renderer with `vite.ssrLoadModule('@sigx/server-renderer/node')` — but naming a package as the **root** of an `ssrLoadModule` call always inlines it into the module runner; `ssr.external` only ever governs an *import* made from inside a module. A project that externalizes the @sigx family (the consumer-shaped dev setup `examples/resume`, `examples/ssr-islands` and `examples/storefront` use) therefore ran the renderer inlined while the app's own `@sigx/*` imports resolved through Node: two live copies of the same file, two sets of `Symbol()` DI tokens, and every app-carried provide dropped on the floor. Since #418 made `app.use(pack())` the one install shape, that meant **`resolveBoundary` never ran in dev** — resumable and island pages rendered with no `__SIGX_BOUNDARIES__` table and no `data-sigx-b` ids at all, so every QRL handler fired against a detached scope (`[sigx resume] … carries no data-sigx-b boundary id`) and reads of resumed state came back undefined. Production was never affected (one Node graph). The handler now loads through `virtual:sigx-ssr-node`, a shim whose whole body is `export * from '@sigx/server-renderer/node'`, putting the package behind an import where the project's external/noExternal decision reaches it — one graph in both setups. A dev server running the handler without the `sigx()` plugin falls back to the direct specifier.
...
- **`@sigx/server-renderer`**: state registered during the stream phase no longer silently drops (#407). The state plugin's streamed emission looked up per-component keys that only `useData`/`useStream` recorded, so anything else landing in the results map during a deferred render never reached the client — a store first created below a streamed boundary hydrated from defaults with no warning, and (same early-return) a keyed `useData` inside `<Defer>` under streaming recorded its key under the CHILD's component id while the resolution hook fired with the Defer's, so the key never shipped and the client silently refetched. Emission now drains a request-level dirty-set (`_unflushedAsyncKeys`, the #279 boundary-table discipline; `_asyncKeysByComponent` is removed), so every registration ships with the next flush regardless of which component was resolving, and a new final drain covers registrations no later flush would carry.

- **`@sigx/runtime-core`**: the DI global-fallback warning no longer misfires on windowless clients (#404). `warnSSRGlobalFallback` gated on `typeof window !== 'undefined'` — the "am I a browser?" check `isLiveClient()` (#204) exists to replace, since lynx's BG thread, the terminal runtime and browser Web Workers have no `window` yet *are* live clients. A lynx app therefore printed, at startup with no server anywhere in the process, that every injectable resolving to its legitimate default was "shared across ALL SSR requests". The gate is now `isLiveClient()`; its `typeof window` fallback keeps web behavior identical when nothing declares.
...
- **Stable data keys — `useData(getVotes)` and fn-ref `invalidates` (#452).** Server functions now carry a build-stamped stable key (`__sigxKey = "<stableId>#<name>"`): `@sigx/vite` emits it on the client stub (new 4th positional) **and** appends it to the SSR module, so both sides key identically. On top of it: **`useData(fn)`** — the data identity IS the function; canonical key `'["<stableId>#<name>"]'`, default fetcher calls the fn (stub on the client, in-process wrapper during SSR) — and **`useData(() => [fn, ...args])`** for parameterized reads (`'["<key>",...args]'`, args feed the fn). `serverFn`'s `invalidates` accepts the same references — bare (`() => [getVotes]`) or inside tuples — resolved by the endpoint to plain stable-key patterns before the wire, so `@sigx/cache` consumes `$cache.invalidates` unchanged and the whole read/invalidate loop is declared with same-module, rename-safe references instead of hand-agreed strings. All fn-derived keys are tuples, so a bare-fn pattern prefix-matches its parameterized reads (`@sigx/cache` `keyMatches` semantics). Outside the Vite transform there is no stamp: `useData(fn)` dev-throws with the remedy, and unstamped `invalidates` refs are dropped with a dev warning.
...
- **`@sigx/runtime-core`**: the `__SIGX_ASYNC__` blob accessors (`peekRestored`/`invalidateRestored`/`writeBack`) now gate on `isLiveClient()` instead of `typeof window` (#407, the requested ruling). The reads always went through `globalThis` — only the gate said "browser" — so a windowless client that declares itself live (lynx, terminal, via `declareLiveClient(true)` at import) can now consume an embedder-installed blob instead of silently missing, and `@sigx/store` deletes its planned second reader of the global (the thing `docs/seams.md` calls a bug). Servers are unchanged: undeclared + windowless is still inert, which is what keeps one request's blob out of another's on a long-lived process. One deliberate edge: a browser embedder that calls `declareLiveClient(false)` now gets inert accessors — the declaration wins in both directions, consistent with #204.

- **`@sigx/runtime-core`**: `defineInjectable` takes per-form options — `{ name }` and `{ hint }` (#404). Both make the DI diagnostics usable by a downstream framework. `defineInjectable(factory, { name })` names an injectable whose factory is an inline arrow (`factory.name` is empty for those, so *every* factory-form injectable used to report as the literal `"sigx:injectable"` and the SSR warning could never say which one it meant); unnamed ones now fall back to their definition site, captured dev-only. `defineInjectable(name, { hint })` replaces the generated `suggestion` on the `SIGX202` error, for packs whose injectable is satisfied by rendering something — `'…must be called from a component rendered as a route by <Stack>'` — rather than by `defineProvide`, which the generated text assumes. The hint is read only in dev builds, but the literal lives in the caller's module, so gate it yourself if the bytes matter; prod still carries just `SIGX202` + the name.

- **`@sigx/server`**: **request context for in-process (SSR-time) calls** (rfc-server §7 v1.1, #309 + #352). The most common server-function shape — `sessionFrom(rq.request)` — worked over RPC and **threw** the moment the same function was called during SSR, since an in-process call has no request to expose. Two ways to supply one now: `fn.with({ context: request })` per call, and `runWithServerFnContext(request, fn)` from `@sigx/server/node`, which uses `AsyncLocalStorage` so every server function called anywhere inside the scope sees it — `serverStream` included, which has no `.with()` channel. Explicit wins over ambient; with neither, the descriptive throw stays, because a function reading `rq.request` when nothing supplied one is a bug worth seeing rather than a silent `undefined`. `node:async_hooks` is imported **dynamically**, so loading `@sigx/server/node` does not pull it on a runtime that lacks it — only calling the function does, and `.with({ context })` is the ALS-free form that works everywhere. The ambient request travels through the new `__SIGX_SERVERFN_CONTEXT__` seam (`docs/seams.md`), re-stamped on every scope entry.

- **Hydration adopts the boundary codec, and the duplicated read paths are united** (#374). A `Date` in SSR'd `useAsync` state arrived on the client as a string, a `Map` as `{}`, an explicit `undefined` vanished — the same failure the RPC wire had, on the other half of the boundary. The state blob and boundary records now encode with `@sigx/serialize` and decode on read, closing the rfc-async §7 cache-seed item.

  The uniting mattered more than the wiring. `@sigx/cache` carried a byte-for-byte copy of runtime-core's blob accessors (`peekBlob`/`writeBlob`, plus an inlined `invalidateRestored`), so "add a decode step" had two answers; it now imports `peekRestored`/`writeBack`/`invalidateRestored` from `@sigx/runtime-core/internals` and never touches the global — which also made the pack **smaller** (2.46 → 2.38 kB). `@sigx/ssr-islands` read `window.__SIGX_BOUNDARIES__` directly rather than through `getBoundaryTable()`, so a decode added to "the" accessor would have silently skipped every island; it now goes through the one accessor at no measured cost. And `record.props` vs `record.state` admitted different values — props consulted registered handlers, state had no handlers parameter at all, so the same `Date` survived as a prop and was dropped with a warning as signal state. Both now route through one `admitPayloadEntry` check that asks the codec (registered handlers **and** built-ins) before falling back to the JSON test.

  Two things that check surfaced: a handler-claimed prop used to skip `isSerializable` entirely, taking the `DANGEROUS_KEYS` rejection with it (a `__proto__` key holding a `Date` went through — defence-in-depth held, since `assignmentJs` targets a null-prototype object, but the check was wrong); and the whole suite passed with the write side emitting tags and no reader decoding them, because nothing tested a `Date` in SSR state at all. Thirteen end-to-end cases now pair each emitter with its reader.

  Decoding deliberately does **not** happen in `getBoundaryTable`/`seedBoundaryState`: both sit in the eager scheduler bundle, whose size entry carries no ignore list precisely to guarantee no runtime reaches the eager path. The eager path reads only boundary metadata (`hydrate`, `media`, `flush`, `chunk`), so decode lives in the lazy chunks that actually mount components — and both eager guards came out marginally *smaller*. Registered handlers reach the client through a new `__SIGX_TYPE_HANDLERS__` seam that `provideTypeHandlers` stamps alongside the DI token (browser only — a process-wide list would let two apps' handlers collide across requests), so packs still make one call.

- **`docs/seams.md`** — a registry of every cross-package `globalThis.__SIGX_*` seam: name, direction, writer, reader, contract, and whether it carries data or control (#374). These globals are how packs coordinate without importing each other, and the map existed only by grepping — which is exactly how the islands boundary read above went unnoticed. `AGENTS.md` now requires an entry for any new seam.
