...
- **`sigx` / `@sigx/runtime-dom`**: the DOM form model processor now ships as its own dist entry (`@sigx/runtime-dom/platform`), named precisely in `sideEffects`; `sigx` is `sideEffects: false` and imports it by subpath. Packages tree-shake fully (unused `Portal`/`useHead`/`show` drop from app bundles) while platform side effects are bundler-proof — previously they survived only by entry-statement retention. (#77)
