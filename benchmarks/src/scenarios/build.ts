/**
 * The sigx scenario trees, extracted from `adapters/sigx.ts` so the
 * request-path benches (`src/micro/packs.ts`) render the SAME trees the
 * comparative SSR suite measures — a pack's overhead is only meaningful
 * against the render it wraps.
 *
 * The adapter still owns the *rendering* (renderToString / renderToNodeStream);
 * this module owns only tree construction.
 */
...
/** Build one scenario's component tree. */
export function build(scenario: ScenarioName): JSXElement {
    switch (scenario) {
